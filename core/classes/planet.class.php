<?php
    ...
    class Planet {
        ...
        public function __construct(int $pID, int $pownerID, string $pname, int $pgala, int $psystem, int $pplanet, int $plast_update, int $pplanet_type, string $pimage,
                                    int $pdiameter, int $pfields_current, int $pfields_max, int $ptemp_min, int $ptemp_max, float $pmetal, float $pcrystal, float $pdeuterium, int $penergy_used,
                                    int $penergy_max, int $pmetal_mine_percent, int $pcrystal_mine_percent, int $pdeuterium_synthesizer_percent, int $psolar_plant_percent, int $pfusion_reactor_percent,
                                    int $psolar_satellite_percent, int $pb_building_id, int $pb_building_endtime, int $pb_tech_id, int $pb_tech_endtime, int $pb_hangar_start_time, string $pb_hangar_id, int $pb_hangar_plus, int $pdestroyed) {
        ...
        public function createPlanet($type,$g = null, $s = null, $p = null) : void {
            ...
            } else if(empty($g) + empty($s) + empty($p) == 3) {
            ...
            while($db->query('SELECT ownerID FROM '.$database['prefix'].'planets WHERE planetID=' . $this->planetID)->rowCount() > 0) {
                $this->planetID = rand(0,100000);
            }
        ...
        public function updatePlanet() : void {
            ...
            if($this->last_update < time()) {
                ...
                $prod_energy = array_sum($data->getUnits()->getEnergyProduction($data->getBuilding()->getSolarPlant(), $data->getBuilding()->getFusionReactor(), $data->getTech()->getEnergyTech(), $data->getFleet()->getSolarSatellite(), $this->temp_max));

                $cons_energy = $this->getMetalMinePercent()/100 * $data->getUnits()->getEnergyConsumption($data->getBuilding()->getMetalMine()) +
                    $this->getCrystalMinePercent()/100 * $data->getUnits()->getEnergyConsumption($data->getBuilding()->getCrystalMine()) +
                    $this->getDeuteriumSynthesizerPercent()/100 * $data->getUnits()->getEnergyConsumption($data->getBuilding()->getDeuteriumSynthesizer()) +
                    $this->getFusionReactorPercent()/100 * $data->getUnits()->getEnergyConsumption($data->getBuilding()->getFusionReactor());
                ...
                if($this->metal < $storage['metal']) {
                    $prod_metal = $prod_factor * $this->getMetalMinePercent() * ($data->getUnits()->getMetalProductionPerHour($lvl_metal)/3600)*$time_diff;
                ...
                if($this->crystal < $storage['crystal']) {
                    $prod_crystal = $prod_factor * $this->getCrystalMinePercent() * ($data->getUnits()->getCrystalProductionPerHour($lvl_crystal)/3600)*$time_diff;
                ...
                if($this->deuterium < $storage['deuterium']) {
                    $prod_deuterium = $prod_factor * $this->getDeuteriumSynthesizerPercent() * ($data->getUnits()->getDeuteriumProductionPerHour($lvl_deuterium)/3600)*$time_diff;
                ...
                if($this->b_building_id > 0 && $this->b_building_endtime > 0 && $this->b_building_endtime <= time()) {
                    ...
                    $stmt = $db->prepare('UPDATE ' . $database['prefix'] . 'buildings SET '.$data->getUnits()->getUnit($this->b_building_id).' = ' . ($level+1));
                ...
                if($this->b_tech_id > 0 && $this->b_tech_endtime > 0 && $this->b_tech_endtime <= time()) {
                    ...
                    // update the building level
                    $stmt = $db->prepare('UPDATE ' . $database['prefix'] . 'techs SET '.$data->getUnits()->getUnit($this->b_tech_id).' = ' . ($level+1));
                ...
                if($this->b_hangar_id != null && $this->b_hangar_id != "" && $this->b_hangar_id != 0) {
                    ...
                    foreach($shipQueueRows as $k => $v) {
                        ...
                        if($unitID > 0 && $unitCnt > 0) {
                            ...
                            $durationForOneUnit =  3600 * $data->getUnits()->getBuildTime($unitID,0,$data->getBuilding()->getRoboticFactory(),$data->getBuilding()->getShipyard(),$data->getBuilding()->getNaniteFactory());
                            ...
                            if($shipFinishedCnt > 0) {
                                ...
                                call_user_func_array(array($data->getFleet(), $method), array($currentShipCount+$shipFinishedCnt));
                                ...
                                // update the building level
                                $stmt = $db->prepare('UPDATE ' . $database['prefix'] . 'fleet SET '.$data->getUnits()->getUnit($unitID).' = ' . ($currentShipCount+$shipFinishedCnt) . ' WHERE planetId = '.$this->planetID);
